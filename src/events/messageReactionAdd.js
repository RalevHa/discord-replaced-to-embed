...
// reply to delete it and restore the original message's native embed — the
// same cleanup messageDelete.js does when the source message itself is
// deleted, just triggered by a reaction instead.
...
module.exports = async function messageReactionAdd(reaction, user, ctx) {
  ...
  if (!originalId) return; // not one of the bot's tracked conversion replies
