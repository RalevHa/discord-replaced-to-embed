...
- [Scraping video/caption/image/engagement out of Facebook's Comet SPA JSON](facebook-comet-hydration-scraping.md) — why "first match on the page" is never safe (order isn't even stable between requests), the video-id/progressive_url/photo_image/feedback-id anchoring that fixes it, and why a video link and a bot embed can't share one Discord message.
